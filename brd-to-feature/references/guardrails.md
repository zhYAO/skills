...
| 10 | btf-init 后顺势开始解析需求/开发 | init 只做环境准备，越界会打乱用户节奏 | init 给完小结即结束，正式流程由用户另行发起 |
...
| 15 | 写 `mcp.json` / `settings.local.json` 失败或无权限时静默跳过、或谎报「已写好」 | 用户以为配好了，实际 MCP/权限没生效，后续阶段无声失败、极难排查 | 🔴 讲清失败原因（token 打码）+ 给手动指引（文件位置 + JSON 片段 + 注意事项），该项标 🟠 待手动；详见 `commands/btf-init.md` Step 2「写入失败 / 无权限兜底」 |
| 16 | **btf-init 时用占位 token（如 `${FIGMA_API_KEY}`）跳过 token 询问** | 写完 mcp.json 就以为 init 通过，实际 token 未填 → 用户后面要用 Figma 时才发现 MCP 401；本轮已经「不静默降级」破戒 | btf-init 第 4 项 figma 是 **必问** 项，无论用户最终选「贴 token」还是「跳过 Figma」，都必须用 `AskUserQuestion` 弹一次。**占位 token ≠ 通过 init**；写入真实值或用户明确说「跳过」后才能把该项标 ✅ |
...
| agent 写 `mcp.json` / `settings.local.json` 失败 / 无权限（`EACCES`、文件不存在、JSON 损坏、环境只读、agent 不支持写） | 换有权限的位置重试（全局↔项目级）或修文件权限后重试 | 把「文件位置 + 待粘 JSON 片段 + 注意事项（合并别覆盖、token 打码）」交用户手动做，该项标 🟠 待手动，**绝不静默跳过/谎报成功**（详见 btf-init Step 2 兜底节） |
