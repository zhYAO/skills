...
#### ⛑️ 写入失败 / 无权限兜底（任何「agent 写配置文件」步骤通用）

下面第 4 / 5 项都涉及 **agent 自动写文件**（MCP 配置 `mcp.json`、免确认配置 `settings.local.json`）。**写入不是总能成功**——可能因为：文件被其它进程占用、目录无写权限、路径不存在、JSON 已损坏无法安全合并、运行环境只读、或当前 agent 根本不暴露写这些文件的能力。

**🔴 红线：写失败必须明说，不许静默降级、不许假装写好了。** 一旦写入失败或没权限，agent **必须**按下面这套兜底走（对应设计原则「不静默降级」）：

1. **先讲清楚发生了什么**：哪个文件、想写什么、为什么失败（贴真实报错，但 token 等敏感值要打码，别回显明文）。
2. **给出手动操作指引**——明确「打开哪个文件 → 粘什么内容 → 注意什么」：

   | 失败情形 | 告诉用户怎么做 |
   |---------|---------------|
   | **目录/文件无写权限**（`EACCES`/`Permission denied`） | 给出文件绝对路径，让用户手动新建/编辑该文件，把 agent 展示的 JSON 片段粘进去；或在有权限的位置重试（如全局写不了就改写项目级 `<项目根>/.claude/`，反之亦然）。需要时提示 `chmod`/改属主，但**不要让 agent 自己 `sudo`**。 |
   | **文件不存在 / 路径不对** | 告诉用户该 agent 配置文件的标准位置（Claude Code `~/.claude/mcp.json`、`~/.claude/settings.local.json`；Cursor `~/.cursor/mcp.json`…），让用户先建文件再粘片段；不确定就让用户查该 agent 文档。 |
   | **JSON 已存在但格式坏 / 合并不安全** | **不要强行覆盖**。把要新增的片段原样给用户，让其手动合并进现有 `mcpServers` / `permissions.allow`，并提示用 JSON 校验器检查。 |
   | **当前 agent 不支持写这些文件** | 直接把完整片段 + 目标文件位置交给用户手动粘贴，并说明这是该 agent 的限制、不是出错。 |
   | **写完仍看不到工具**（写成功但 MCP 未加载） | 让用户重载 / 重启 agent；仍无则核对片段里的 server 名、命令、`env` 是否正确，按 `references/figma.md` / `references/miniprogram-testing.md` 排查。 |

3. **把对应项标成 🟠 待你手动**（不是 ✅），并在 Step 3 快照里如实反映。
4. **敏感值处理**：手动指引里如含 token，提示用户自己粘真实值、别把明文发回对话；agent 展示片段时对已知 token 打码。

> 一句话：**能自动写就自动写，写不了就把「文件位置 + 片段 + 注意事项」清清楚楚交给用户手动做**，绝不静默跳过、绝不谎报成功。下面各步只在正常路径上描述「agent 写入」，**失败一律回到本节**。
...
> ⛑️ **写 `mcp.json` 失败 / 无权限怎么办** → 回到本节开头「写入失败 / 无权限兜底」：agent 必须讲清失败原因（token 打码）、把上面这段 JSON + 目标文件路径交给你手动粘贴，把本项标成 🟠 待手动，**不许静默跳过或谎报已写**。
...
> ⛑️ **写 `mcp.json` 失败 / 无权限怎么办** → 回到本节开头「写入失败 / 无权限兜底」：把上面这段 weapp-dev 片段 + 目标文件路径交给你手动粘贴，本项标 🟠 待手动，不静默跳过。
...
> ⛑️ **写 `settings.local.json` 失败 / 无权限怎么办** → 回到本节开头「写入失败 / 无权限兜底」：agent 讲清失败原因、把这段 `permissions.allow` 片段 + 目标文件路径交给你手动粘贴（已有文件则手动合并进 `allow` 数组、别覆盖），本项标 🟠 待手动，不静默跳过。
...
| 🟠 配置写入失败 / 无权限 | 按 agent 给的「文件位置 + JSON 片段 + 注意事项」手动粘贴/合并（详见 Step 2 开头「写入失败 / 无权限兜底」）；权限问题就改写有权限的位置或修文件权限后重试 |
...
5. **写配置失败不许静默降级 / 谎报成功**——`mcp.json` / `settings.local.json` 写入失败或无权限时，agent 必须讲清原因 + 给手动指引（文件位置 + 片段 + 注意事项，token 打码），把该项标 🟠 待手动，按 Step 2 开头「写入失败 / 无权限兜底」处理。
6. **不要顺势开始解析需求** —— init 到此为止，正式流程另起。
