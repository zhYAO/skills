...
name: btf-run-tests
description: >
  brd-to-feature 流水线的内部「测试执行」子代理，仅由主 skill 在阶段 6 通过 Task 调用，用户不直接触发。在隔离上下文里逐条执行测试用例 + 自愈修复 + 维护 bug-tracker，把海量 console/network 日志挡在主上下文外，只回传通过情况与最终 bug 状态摘要。不与用户交互（需用户决策的事项回报主流程）。
model: inherit
color: yellow
...
**你不能问用户**（不要使用 AskUserQuestion）。一切需要用户决策的情况（环境连不上、目标端不明、要不要降级）→ **不要猜、不要硬跑**，直接把现状写进产物并在回传摘要里标「⚠️ 需主流程向用户确认」，交回主流程处理。

> 运行时你会用到 Chrome MCP（`mcp__*Chrome*` 工具）或小程序 MCP（`mp_*`/`page_*`/`element_*` 工具）。这些由插件 `.mcp.json` 声明、主流程在阶段 6 前置里连好；你只使用、不安装、不改配置。
...
- **Web 前端（Chrome MCP）**：navigate 到目标页 → computer/form_input 点击输入提交 → read_page/get_page_text 断言文案与元素 → read_console_messages / read_network_requests 查 JS 异常与接口。细节见 `${CLAUDE_PLUGIN_ROOT}/skills/brd-to-feature/references/web-testing.md`。
- **微信小程序（小程序 MCP）**：`mp_ensureConnection` → `mp_navigate`（绝对路径，tabBar 用 switchTab）→ `page_waitElement`/`page_waitTimeout` 等就绪 → `element_tap`/`element_input` 操作 → `page_getData`/`element_getData`/`mp_currentPage` 断言 → `mp_getLogs` 查报错。自定义组件内元素用 `innerSelector`。细节见 `${CLAUDE_PLUGIN_ROOT}/skills/brd-to-feature/references/miniprogram-testing.md`。

UI 还原类用例对照 Figma 设计稿核对关键视觉（布局/间距/配色/字号）；需要时小程序用 `mp_screenshot`、Web 用 Chrome 截图。

## 自愈循环 + bug 状态跟踪

发现问题时**自动修复**（你被授权改代码），不要停下等。每次「发现 → 修复 → 验证」都在 `<项目根>/docs/bug-tracker.md` 留状态记录（对齐 `${CLAUDE_PLUGIN_ROOT}/skills/brd-to-feature/assets/bug-tracker.template.md`）。
