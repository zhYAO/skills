...
若没有任何 figma 工具，**不要假装能看到设计稿**。本插件已声明 Framelink MCP（见下），通常只是**还没设 token 环境变量**；用户若明确表示没有设计稿，可跳过设计稿、仅按需求文档开发。

## 缺失时：设 Figma token（插件已声明 Framelink）

本插件的 `.mcp.json` 已声明社区 Framelink MCP（`figma`，走 `npx -y figma-developer-mcp --stdio`），安装插件即注册——**不需要手改 agent 的 MCP 配置**。它要的只是一个 Figma API token，通过环境变量 `FIGMA_API_KEY` 注入（`.mcp.json` 里写的是 `${FIGMA_API_KEY}`）。

明确告诉用户怎么拿 token：Figma → `Settings` → `Security` → `Personal access tokens` → 生成，权限至少给 File content 读取。拿到后设到环境变量（示例，按用户 shell/平台为准）：

```bash
export FIGMA_API_KEY="<用户的 Figma access token>"
```

不要把 token 写进会被提交的文件或打印到日志。设好后一般要让 agent 重新加载 MCP 配置才能看到 `figma` 工具。Framelink 具体形态可能随版本变化，以 [Framelink 官方文档](https://github.com/GLips/Figma-Context-MCP) 为准。
...
> 备选：如果用户有 Figma 付费席位、偏好官方 Dev Mode MCP（免 token，内置在 Figma 桌面应用，开启后走本地 `http://127.0.0.1:3845/sse`），也可用；但**默认走插件已声明的 Framelink**，仅在用户明确要求时改用 Dev Mode。
