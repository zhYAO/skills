...
2. **小程序 MCP（测微信小程序需要）** — 检测有没有 `wechat_*` 工具。没有就**问用户是否现在装**，同意后按 `references/miniprogram-testing.md` 的「缺失检查 + 自动安装」执行（`pip install uv` → `uv tool install wechat-devtools-mcp` → 配置 + 两个环境变量 + 开启服务端口）。
3. **Figma MCP（按设计稿开发需要）** — 检测有没有 `figma` 工具。没有就引导用户在两种方案里选一种（见 `references/figma.md`）：官方 Dev Mode（免装包，需 Figma 付费席位开启）或社区 Framelink（npx + **Figma access token**）。选 Framelink 时，明确告诉用户 token 获取路径：Figma → `Settings` → `Security` → `Personal access tokens` → 生成，权限至少给 File content 读取；拿到后填进 MCP 配置的 `FIGMA_API_KEY`，别提交进仓库或打日志。
